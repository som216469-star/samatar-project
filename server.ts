...
const transporter = nodemailer.createTransport({
  ...
  secure: true,
  auth: {
    user: process.env.SMTP_USER || "samataromer4@gmail.com",
    pass: process.env.SMTP_PASS || "gqhfdqimvdhmgkmm",
  },
});
...
// ✅ NODEMAILER EMAIL FUNCTION - Port 465
async function sendVerificationEmail(toEmail: string, code: string) {
  try {
    await transporter.sendMail({
      from: `"Dugsiga Pro" <${process.env.SMTP_USER || "samataromer4@gmail.com"}>`,
      to: toEmail,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
          ...
          <p>Saxeexkaaga ama Signup-kaaga si loo xaqiijiyo, fadlan isticmaal lambarkan xaqiijinta:</p>
  ...
  if (!useLocalFallback) {
    try {
      ...
      const { error } = await supabase.from("dugsiga_users").insert([{
        email, password: passwordHash, verified: false, verification_code: verificationCode
      }]);
    ...
    if (existingUser) return res.status(400).json({ error: "Email-kan horey ayaa loo diiwaangeliyey." });
    db.users.push({ email, password_hash: passwordHash, verified: false, verification_code: verificationCode });
    saveLocalDB(db);
  }
  const emailSent = await sendVerificationEmail(email, verificationCode);
  res.json({ success: true, message: "Diiwaangelintu way guuleysatay! Waxaa laguu soo diray koodhka xaqiijinta.", emailSent });
});

app.post("/api/auth/verify", async (req, res) => {
  const { email, code } = req.body;
  if (!email || !code) return res.status(400).json({ error: "Email iyo Koodh xaqiijin ah ayaa loo baahan yahay." });
  if (!useLocalFallback) {
    try {
      const { data: user, error } = await supabase.from("dugsiga_users").select("*").eq("email", email).single();
      if (error) {
        if (error.code === "PGRST116") return res.status(400).json({ error: "Isticmaalaha lama helin." });
        throw error;
      }
      if (!user) return res.status(400).json({ error: "Isticmaalaha lama helin." });
      if (user.verification_code !== code) return res.status(400).json({ error: "Koodhka xaqiijintu waa qalad." });
      const { error: updateError } = await supabase.from("dugsiga_users").update({ verified: true }).eq("email", email);
    ...
    if (userIndex === -1) return res.status(400).json({ error: "Isticmaalaha lama helin." });
    if (db.users[userIndex].verification_code !== code) return res.status(400).json({ error: "Koodhka xaqiijintu waa qalad." });
    db.users[userIndex].verified = true;
    saveLocalDB(db);
  }
  res.json({ success: true, message: "Akoonkaaga si guul leh ayaa loo xaqiijiyey! Hadda waad geli kartaa." });
});
...
app.post("/api/auth/login", async (req, res) => {
  ...
  if (!userRecord.verified) return res.status(400).json({ error: "Akoonkan weli lama xaqiijin.", unverified: true });
...
app.get("/api/students", async (req, res) => {
  if (!useLocalFallback) {
    try {
      const { data, error } = await supabase.from("dugsiga_students").select("*").order("full_name", { ascending: true });
      if (error) throw error;
      const students = data.map(s => ({ id: s.id, fullName: s.full_name, class: s.class, gender: s.gender, guardianPhone: s.guardian_phone, status: s.status, createdAt: s.created_at }));
...
app.post("/api/students", async (req, res) => {
  ...
  if (!useLocalFallback) {
    try {
      const { data: existing, error: checkError } = await supabase.from("dugsiga_students").select("id").ilike("full_name", student.fullName.trim()).eq("class", student.class).limit(1);
      ...
      const { error } = await supabase.from("dugsiga_students").insert([{ id: student.id, full_name: student.fullName.trim(), class: student.class, gender: student.gender, guardian_phone: student.guardianPhone, status: student.status, created_at: student.createdAt }]);
      if (error) throw error;
      return res.json(student);
    } catch (e: any) { return handleSupabaseError(res, e, "Diiwaangelinta Ardayga (Add Student)"); }
  } else {
    const db = loadLocalDB();
    const isDuplicate = db.students.some(s => s.fullName.trim().toLowerCase() === student.fullName.trim().toLowerCase() && s.class === student.class);
    ...
    db.students.push({ ...student, fullName: student.fullName.trim() });
    saveLocalDB(db);
    res.json(student);
  }
});
...
app.put("/api/students/:id", async (req, res) => {
  ...
  if (!useLocalFallback) {
    try {
      const { error } = await supabase.from("dugsiga_students").update({ full_name: updates.fullName, class: updates.class, gender: updates.gender, guardian_phone: updates.guardianPhone, status: updates.status }).eq("id", id);
      if (error) throw error;
...
app.post("/api/attendance", async (req, res) => {
  ...
  if (!useLocalFallback) {
    try {
      try {
        const { error: delError } = await supabase.from("dugsiga_attendance").delete().eq("date", date).eq("session_type", sType);
        if (delError) {
          if (delError.code === '42703' || (delError.message && delError.message.toLowerCase().includes('session_type'))) {
            const { error: delError2 } = await supabase.from("dugsiga_attendance").delete().eq("date", date);
      ...
      const dbRecords = records.map(r => ({ date: date, student_id: r.studentId, status: r.status, timestamp: r.timestamp }));
    ...
    records.forEach(r => { db.attendance.push({ date, studentId: r.studentId, status: r.status, timestamp: r.timestamp, sessionType: sType }); });
    saveLocalDB(db);
    res.json({ success: true });
  }
});
...
app.post("/api/fees", async (req, res) => {
  ...
  if (!useLocalFallback) {
    try {
      const { data: existing, error: checkError } = await supabase.from("dugsiga_fees").select("id").eq("student_id", fee.studentId).eq("month", fee.month).eq("year", fee.year).limit(1);
      if (checkError) throw checkError;
      ...
      const { error } = await supabase.from("dugsiga_fees").insert([{ id: fee.id, student_id: fee.studentId, month: fee.month, year: fee.year, amount: fee.amount, paid_amount: fee.paidAmount, status: fee.status, created_at: fee.createdAt, updated_at: fee.updatedAt, history: fee.history }]);
      if (error) throw error;
      return res.json(fee);
    } catch (e: any) { return handleSupabaseError(res, e, "Abuurista Biilka (Create Fee)"); }
  } else {
    const db = loadLocalDB();
    const isDuplicate = db.fees.some(f => f.studentId === fee.studentId && f.month === fee.month && f.year === fee.year);
...
app.put("/api/fees/:id", async (req, res) => {
  ...
  if (!useLocalFallback) {
    try {
      const { error } = await supabase.from("dugsiga_fees").update({ month: updates.month, year: updates.year, amount: updates.amount, paid_amount: updates.paidAmount, status: updates.status, updated_at: updates.updatedAt, history: updates.history }).eq("id", id);
      if (error) throw error;
      return res.json({ success: true });
...
app.post("/api/classes", async (req, res) => {
  ...
  if (!useLocalFallback) {
    try {
      const { data: existing, error: checkError } = await supabase.from("dugsiga_classes").select("id").ilike("class_name", className.trim()).limit(1);
      if (checkError) throw checkError;
      ...
      const { error } = await supabase.from("dugsiga_classes").insert([{ id: id || 'cls-' + Math.random().toString(36).substr(2, 9), class_name: className.trim(), teacher_name: teacherName || "", room_number: roomNumber || "", description: description || "", created_at: createdAt || new Date().toISOString().split('T')[0] }]);
      if (error) throw error;
      return res.json({ success: true });
    ...
    const isDuplicate = db.classes.some(c => c.className.trim().toLowerCase() === className.trim().toLowerCase());
    if (isDuplicate) return res.status(400).json({ error: "Fasalkan magacan leh horey ayaa loo diiwaangeliyey. (Duplicate Class)" });
    db.classes.push({ id: id || 'cls-' + Math.random().toString(36).substr(2, 9), className: className.trim(), teacherName: teacherName || "", roomNumber: roomNumber || "", description: description || "", createdAt: createdAt || new Date().toISOString().split('T')[0] });
    saveLocalDB(db);
    res.json({ success: true });
  }
});
...
app.put("/api/classes/:id", async (req, res) => {
  ...
  if (!useLocalFallback) {
    try {
      const { error } = await supabase.from("dugsiga_classes").update({ class_name: className, teacher_name: teacherName, room_number: roomNumber, description: description }).eq("id", id);
      if (error) throw error;
      return res.json({ success: true });
...
app.post("/api/subjects", async (req, res) => {
  ...
  if (!useLocalFallback) {
    try {
      const { data: existing, error: checkError } = await supabase.from("dugsiga_subjects").select("id").ilike("subject_name", subjectName.trim()).eq("class_name", className || "").limit(1);
      if (checkError) throw checkError;
      ...
      const { error } = await supabase.from("dugsiga_subjects").insert([{ id: id || 'sub-' + Math.random().toString(36).substr(2, 9), subject_name: subjectName.trim(), subject_code: subjectCode || "", class_name: className || "", teacher_name: teacherName || "", created_at: createdAt || new Date().toISOString().split('T')[0] }]);
      if (error) throw error;
      return res.json({ success: true });
    ...
    const isDuplicate = db.subjects.some(s => s.subjectName.trim().toLowerCase() === subjectName.trim().toLowerCase() && s.className === (className || ""));
    if (isDuplicate) return res.status(400).json({ error: "Maaddadan magacan leh horey ayaa loogu daray fasalkan. (Duplicate Subject)" });
    db.subjects.push({ id: id || 'sub-' + Math.random().toString(36).substr(2, 9), subjectName: subjectName.trim(), subjectCode: subjectCode || "", className: className || "", teacherName: teacherName || "", createdAt: createdAt || new Date().toISOString().split('T')[0] });
    saveLocalDB(db);
    res.json({ success: true });
  }
});
...
app.put("/api/subjects/:id", async (req, res) => {
  ...
  if (!useLocalFallback) {
    try {
      const { error } = await supabase.from("dugsiga_subjects").update({ subject_name: subjectName, subject_code: subjectCode, class_name: className, teacher_name: teacherName }).eq("id", id);
      if (error) throw error;
      return res.json({ success: true });
...
app.post("/api/exams", async (req, res) => {
  ...
  if (!useLocalFallback) {
    try {
      const { data: existing, error: checkError } = await supabase.from("dugsiga_exam_scores").select("id").eq("student_id", studentId).eq("subject_name", subjectName).eq("exam_name", examName).limit(1);
      if (checkError) throw checkError;
    ...
    const isDuplicate = db.examScores.some(e => e.studentId === studentId && e.subjectName === subjectName && e.examName === examName);
    ...
    db.examScores.push({ id: id || 'exm-' + Math.random().toString(36).substr(2, 9), studentId, studentName: studentName || "", className, subjectName, examName, term: term || "Term 1", maxMarks: maxMarks || 100, marksObtained, grade: grade || "", examDate: examDate || new Date().toISOString().split('T')[0], createdAt: createdAt || new Date().toISOString().split('T')[0] });
    saveLocalDB(db);
    res.json({ success: true });
  }
});
...
app.put("/api/exams/:id", async (req, res) => {
  ...
  if (!useLocalFallback) {
    try {
      const { error } = await supabase.from("dugsiga_exam_scores").update({ student_id: studentId, student_name: studentName, class_name: className, subject_name: subjectName, exam_name: examName, term: term, max_marks: maxMarks, marks_obtained: marksObtained, grade: grade, exam_date: examDate }).eq("id", id);
      if (error) throw error;
      return res.json({ success: true });
...
app.get("/api/settings", async (req, res) => {
  if (!useLocalFallback) {
    try {
      const { data, error } = await supabase.from("dugsiga_settings").select("*").eq("key", "main_settings").single();
...
app.put("/api/settings", async (req, res) => {
  ...
  if (!useLocalFallback) {
    try {
      const { error } = await supabase.from("dugsiga_settings").upsert({ key: "main_settings", value: settings });
      if (error) throw error;
      return res.json({ success: true });
...
app.post("/api/reset", async (req, res) => {
  ...
  const initial: LocalDB = { users: [], students: [], attendance: [], fees: [], classes: [], subjects: [], examScores: [], settings: defaultSettings };
  fs.writeFileSync(LOCAL_DB_PATH, JSON.stringify(initial, null, 2));
  res.json({ success: true });
});
